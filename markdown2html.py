...
import subprocess
import tempfile
styles = open("styles.html", "r").read()
fonts = open("fonts.html", "r").read()

def markdown2html(markdown):
  html = originalmarkdown2html(markdown)
  tmpfile = tempfile.NamedTemporaryFile()
  open(tmpfile.name, 'a+').write(styles+html)
  finlist = []
  ok = False
  for x in subprocess.check_output(["node", "inliner.js", tmpfile.name]).decode("utf-8").strip().split("\n"):
    if x == "</style>":
      ok = True
      continue
    if ok:
